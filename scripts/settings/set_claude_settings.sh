...
function set_claude_settings() {
  ...
  if [[ ! -d "${claude_config_dir}" ]]; then
    report_action_taken_to_log "Claude support directory not found; launching Claude once to seed config"
    launch_and_quit_app "${BUNDLE_ID_CLAUDE}" ; success_or_not
  fi

  report_action_taken_to_log "Ensure Claude config directory exists: ${claude_config_dir}"
  ...
  if [[ ! -f "${claude_config_path}" || ! -s "${claude_config_path}" ]]; then
    report_action_taken_to_log "Initialize Claude config file if missing/empty: ${claude_config_path}"
  ...
  report_action_taken_to_log "Apply Claude preference updates via jq"
  jq "${jq_filter}" "${claude_config_path}" > "${tmpfile}" ; success_or_not

  report_action_taken_to_log "Save updated Claude config to ${claude_config_path}"
