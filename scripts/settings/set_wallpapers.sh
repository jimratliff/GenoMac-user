...
function set_wallpapers_for_all_spaces() {
  ...
  for (( space_number=1; space_number <= MAXIMUM_NUMBER_OF_MISSION_CONTROL_SPACES; ++space_number )); do
    move_to_mission_control_space_by_number "$space_number"
    wallpaper_path="$(get_path_to_wallpaper_for_mission_control_space_by_number "$space_number")"
    report_to_log "Space ${space_number} path: ${wallpaper_path}"
    set_all_displays_of_current_mission_control_space_to_image_at_path "$wallpaper_path"
  done

  # Return to Space #1
  move_to_mission_control_space_by_number 1
  
  report_end_phase_standard
}

function get_path_to_wallpaper_for_mission_control_space_by_number() {
...
function move_to_mission_control_space_by_number() {
  ...
  report_start_phase "Entering move_to_mission_control_space_by_number : $*"
...
APPLESCRIPT
  ...
  report_end_phase "Leaving move_to_mission_control_space_by_number : $*"
}
