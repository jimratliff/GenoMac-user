...
function conditionally_transfer_system_scoped_user_attribute_states_to_user_scoped() {
  # Transfer system-scoped user-attribute states to user scoped,
  # if this hasn’t been done already this session.
...
function set_user_preferences_for_attribute() {
  ...
  case "$attribute_name" in
    "${USER_ATTRIBUTE_TOUCH_ID_ROOT}"*)
      ...
      ;;
    "${USER_ATTRIBUTE_BAREBONES}")
      report_action_taken_to_log "Setting preferences for attribute: ${USER_ATTRIBUTE_BAREBONES}"
      set_genomac_user_state "$SESH_USER_WANTS_ONLY_BAREBONES_CONFIG"
      ;;
    ...
    "${USER_ATTRIBUTE_SYNC_COM}")
      ...
      ;;
    "${USER_ATTRIBUTE_WALLPAPERS}")
      report_action_taken_to_log "Setting preferences for attribute: ${USER_ATTRIBUTE_WALLPAPERS}"
