...
function ensure_zshenv_loaded() {
  ...
  report_start_phase_standard
  report_action_taken_to_log "Ensuring that ~/.zshenv is loaded"
...
function stow_dotfiles_for_each_package() {
  ...
  for package in "${GMU_ARRAY_OF_PACKAGES_TO_STOW_DOTFILES[@]}"; do
    report_action_taken_to_log "Stowing package: $package"
  ...
  (
    report_action_taken_to_log "Resetting GenoMac-user repo to discard any adopted file content"
...
function prepare_zsh_sessions_for_outside_zdotdir() {
  ...
  report_start_phase_standard
  report_action_taken_to_log "Preparing Zsh sessions to be stored outside of .config"
  ...
  report_action_taken_to_log "Ensuring state dirs: '$XDG_ZSH_STATE_DIR' & '$XDG_ZSH_SESSIONS_DIR'"
  ...
  if [[ ! -e "$HISTFILE" ]]; then
    report_action_taken_to_log "Creating empty history '$HISTFILE' (0600)"
    umask 077
    : > "$HISTFILE"
    chmod 600 "$HISTFILE"
    success_or_not
  else
    report_action_taken_to_log "History exists at '$HISTFILE' — leaving as-is"
  fi
  ...
  report_action_taken_to_log "Linking '$ZDOT_SESS_PATH' -> '$XDG_ZSH_SESSIONS_DIR' (idempotent)"
