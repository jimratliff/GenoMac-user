...
#   - The command to which the hotkey is assigned is specified by its integer ID
#	  (e.g., 118 is "Switch to Desktop 1")
#   - The key is specified by a case-insensitive key name, e.g., e.g., "F3", "a", "E", "up-arrow", etc.
#   - The modifiers are a string of concatenated instances of SHIFT_CHAR, CONTROL_CHAR, OPTION_CHAR, 
# 	  COMMAND_CHAR, or, alternatively, META_MODIFIER_CHARS
...
function disable_command_by_its_id() {
  ...
  report_adjust_setting "Disable command #${command_ID}: $command_description"
  modify_symbolichotkeys_entry_for_command_by_id $command_ID "${XML_TO_DISABLE_COMMAND}" ; success_or_not
}
...
function assign_hotkey_to_command_id() {
  ...
  ############### DEBUG
  report "Inside assign_hotkey… $command_ID $key_name $modifier_chars"
