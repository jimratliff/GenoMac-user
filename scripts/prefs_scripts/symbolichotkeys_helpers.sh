...
function get_hotkey_ascii_and_AppleScript_key_codes() {
  ...
  # Returns: "ascii_code applescript_code" or "ERROR" if key not found
    ...
    if [[ -z "$key" ]]; then
        echo "Error: get_key_codes requires a key description argument" >&2
        return 1
    fi
    ...
    if [[ -n ${ascii_codes[$key_lower]} ]]; then
        ...
        echo "Error: Unknown key description '$key'" >&2
        return 1
    fi
}
...
# Example usage:
# get_key_codes "a"        # Returns: "97 0"
# get_key_codes "F5"       # Returns: "65535 96"
# get_key_codes "space"    # Returns: "32 49"
# get_key_codes "up-arrow" # Returns: "65535 126"
