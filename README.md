...
USER_CONFIGURER is the designated user for performing systemwide configurations. In particular, USER_CONFIGURER is the only user authorized to use Homebrew to add or update applications managed by Homebrew.[^HOMEBREW_UPDATE_IS_DIFFERENT] If USER_CONFIGURER of a Mac has already been created and configured and you’re starting to configure an additional user, the current GenoMac-user repo *is* the correct place to start.
