...
[^common_across_users]: The set of generic user-scoped settings won’t exhaust all user-scoped settings. Any user is free to choose their own setting for any parameter not covered by the generic settings. However, this raises a nuanced distinction. Each Mac has, for example, a USER_VANILLA. Suppose it is desired that USER_VANILLA has a different choice of some setting than all other users. Nevertheless, it could well be advantageous for that setting to be consistent across all Macs in the sense that, every USER_VANILLA shares that same choice, regardless of which Mac they inhabit. When such a case arises, there could be further refactoring of the GenoMac-user framework so that each user *type* (e.g., USER_VANILLA, USER_CONFIGURER, or USER_DEV) has its own configurations that would be overlain on the common-across-all-users generic settings. This possibility would apply whether the parameter at issue (a) was not part of the set of generic settings or (b) is covered by, and conflicts with, the prescriptions of the generic settings. In the latter case, applying USER_VANILLA’s choices *after* the generic settings would have the effect of overriding for USER_VANILLA the generic settings. (TODO: To be fleshed out: this is now implemented through the user of user attributes.)

[^genomac_shared_purpose]: GenoMac-shared is an externally defined set of common code that specifies some environment variables and defines some helper functions. This common code is incorporated into each of GenoMac-system and GenoMac-user as a submodule located at `external/genomac-shared` of each of the two container repositories. See GenoMac-shared’s [README](https://github.com/jimratliff/GenoMac-shared/blob/main/README.md) for information on how that affects/complicates work flows, particularly how to update GenoMac-system and GenoMac-user when there is a change to GenoMac-shared’s code.

[^slightly_sensitive_data]: To be clear, secrets are *not* exposed in GenoMac-private. GenoMac-private contains user *names*, volume names, and attributes assigned to users, which attributes can indicate particular applications or other characteristics of a user’s workflow. These are not sensitive in the sense that secrets are, but there’s no reason to give unwarranted external visibility into details of a installation. For example, OWASP [warns against disclosing “unnecessary system details](https://owasp.org/www-project-secure-coding-practices-quick-reference-guide/stable-en/02-checklist/05-checklist#:~:text=Do%20not%20store%20sensitive%20information%20in%20logs%2C%20including%20unnecessary%20system%20details%2C),” [including usernames](https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/03-Identity_Management_Testing/04-Testing_for_Account_Enumeration_and_Guessable_User_Account?utm_source=chatgpt.com), and [implementation details in general](https://owasp.org/www-community/Improper_Error_Handling#:~:text=These%20messages%20reveal%20implementation%20details%20that%20should%20never%20be%20revealed%2E%20Such%20details%20can%20provide%20hackers%20important%20clues%20on%20potential%20flaws%20in%20the%20site).

Both the GenoMac-system and GenoMac-user repositories are intended to be cloned locally as the mechanism for providing access to the necessary scripts and other resources:
- GenoMac-system is cloned only by the designated configuring user, USER_CONFIGURER, for that Mac.
- GenoMac-user is cloned separately by *each user* being configured by Project GenoMac.
...
The Hypervisor is a scripting function that manages the configuration of the user, both (a) for the initial bootstrap and (b) for occasional maintenance.
...
### Run the Hypervisor
The Hypervisor is a scripting system that manages the configuration of the user, both (a) for the initial bootstrap and (b) for occasional maintenance.
...
### When to run the Hypervisor for occasional maintenance
...
- Matrix screensaver and hot-corner activation of screen saver
  ...
  - Update January 20, 2026: Update (macOS 26.2): The screensaver seems to work fine when trigged by elapsed time, but all I get is a blank screen when I try to trigger the screensaver via hot corner. 
...
See [Dev issues common to both GenoMac-system and GenoMac-user](https://github.com/jimratliff/GenoMac-shared/blob/main/docs/shared_dev_issues.md).
